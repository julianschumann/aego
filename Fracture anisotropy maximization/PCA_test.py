...
data_ae = np.load('Results' + Path_add_on + '/Dimension.npy', allow_pickle = True)
...
np.save('Results' + Path_add_on + '/PCA.npy', data)
np.save('Results' + Path_add_on + '/PCA_rand.npy', data_comp)



#%% Save results to text for paper figure 
fpca=open('Results' + Path_add_on + '/f_PCA_loss' + Path_add_on + '.txt','w+')
fpca.write('m L Lr \n')
for k in range(len(Edim)):
    fpca.write('{:10.3e} {:10.3e} {:10.3e}\n'.format(Edim[k], loss[k], loss_comp[k]))
fpca.close()

fae=open('Results' + Path_add_on + '/f_AE_loss' + Path_add_on + '.txt','w+')
fae.write('m L \n')
for k in range(len(Edim_ae)):
    fae.write('{:10.3e} {:10.3e}\n'.format(Edim_ae[k], loss_ae[k]))
fae.close()

fvar=open('Results' + Path_add_on + '/f_PCA_var' + Path_add_on + '.txt','w+')
fvar.write('m V Vr \n')
