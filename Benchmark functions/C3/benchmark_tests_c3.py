...
def cost_dec(Z,decoder,xmin,xmax):
    X=decoder.predict(Z)
...
def one_run(num_samples, number_advances,dim,edim,xmin,xmax,X_in):
    ...
    C[0,:]=cost_dec(Z,decoder,xmin,xmax)
    ...
    for i in range(imax):
        ...
        Zcom[Zcom<0]=0
        Zcom[Zcom>1]=1  
        Ccom=cost_dec(Zcom,decoder,xmin,xmax)
