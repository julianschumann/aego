...
def cost(X,dim):
    dim=np.arange(dim)
    dim_vector=np.tile(dim[np.newaxis,:],(len(X),1))
    C=1+1/4000*np.sum(X**2,1)-np.prod(np.cos(X/np.sqrt(dim_vector+1)),1)
    return C
...
for i_e,edim in enumerate(Edim):
    ...
    fae=open('f_4ae_{}.txt'.format(edim),'w+')
    fae.write('n c \n')
    ...
    fpost=open('f_4post_{}.txt'.format(edim),'w+')
    fpost.write('n c \n')
